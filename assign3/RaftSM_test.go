...
func TestFollowerAppendEntriesRequest(t *testing.T) {
	...
	expectAction(t, action, []interface{}{Alarm{200}, StateStore{"FOLLOWER", 3, 0}, Commit{index: 0, data: []byte("read"), err: nil}, LogStore{0, LogEntry{1, []byte("read")}}, LogStore{1, LogEntry{2, []byte("cas")}}, LogStore{2, LogEntry{3, []byte("write")}}, Send{1, AppendEntriesResponseEvent{2, 3, true}}}, "Error in append entries request follower")
	...
	config = Config{4, []int{1, 2, 4, 5}}
	sm = StateMachine{myconfig: config, state: "FOLLOWER", currentTerm: 2, votedFor: 1, log: []LogEntry{{1, []byte("read")}, {2, []byte("cas")}, {3, []byte("delete")}}, logCurrentIndex: 2, logCommitIndex: 0}
	newlog = []LogEntry{{1, []byte("read")}, {2, []byte("cas")}, {4, []byte("write")}, {4, []byte("delete")}}
	action = sm.ProcessEvent(AppendEntriesRequestEvent{term: 4, leaderId: 1, prevLogIndex: 1, prevLogTerm: 2, data: []LogEntry{{4, []byte("write")}, {4, []byte("delete")}}, leaderCommitIndex: 2})
	expectStateMachine(t, sm, StateMachine{myconfig: config, state: "FOLLOWER", currentTerm: 4, votedFor: 0, log: newlog, logCurrentIndex: 3, logCommitIndex: 2}, "Error in FollowerAppendEntriesRequest-case3")
	expectAction(t, action, []interface{}{Alarm{200}, StateStore{"FOLLOWER", 4, 0}, Commit{index: 1, data: []byte("cas"), err: nil}, Commit{index: 2, data: []byte("write"), err: nil}, LogStore{2, LogEntry{4, []byte("write")}}, LogStore{3, LogEntry{4, []byte("delete")}}, Send{1, AppendEntriesResponseEvent{4, 4, true}}}, "Error in FollowerAppendEntriesRequest-case5")
...
func TestLeaderTimeout(t *testing.T) {
	...
	expectAction(t, action, []interface{}{Alarm{100}, Send{2, AppendEntriesRequestEvent{3, 1, 0, 1, []LogEntry{{2, []byte("cas")}, {3, []byte("write")}}, 0}}, Send{3, AppendEntriesRequestEvent{3, 1, 0, 1, []LogEntry{{2, []byte("cas")}, {3, []byte("write")}}, 0}}, Send{4, AppendEntriesRequestEvent{3, 1, 1, 2, []LogEntry{{3, []byte("write")}}, 0}}, Send{5, AppendEntriesRequestEvent{3, 1, 0, 1, []LogEntry{{2, []byte("cas")}, {3, []byte("write")}}, 0}}}, "Error in leader timeout")

	// Timeout leader with first log entry at the follower
	sm = StateMachine{myconfig: config, state: "LEADER", currentTerm: 3, log: mylog, nextIndex: []int{0, 1, 2, 1}, logCurrentIndex: 1, logCommitIndex: 0}
	action = sm.ProcessEvent(TimeoutEvent{})
	expectStateMachine(t, sm, StateMachine{myconfig: config, state: "LEADER", currentTerm: 3, log: mylog, nextIndex: []int{0, 1, 2, 1}, logCurrentIndex: 1, logCommitIndex: 0}, "Error in leader timeout")
	expectAction(t, action, []interface{}{Alarm{100}, Send{2, AppendEntriesRequestEvent{3, 1, -1, 0, []LogEntry{{1, []byte("read")}, {2, []byte("cas")}, {3, []byte("write")}}, 0}}, Send{3, AppendEntriesRequestEvent{3, 1, 0, 1, []LogEntry{{2, []byte("cas")}, {3, []byte("write")}}, 0}}, Send{4, AppendEntriesRequestEvent{3, 1, 1, 2, []LogEntry{{3, []byte("write")}}, 0}}, Send{5, AppendEntriesRequestEvent{3, 1, 0, 1, []LogEntry{{2, []byte("cas")}, {3, []byte("write")}}, 0}}}, "Error in leader timeout")
...
func TestVoteRequestFollower(t *testing.T) {
	config := Config{2, []int{1, 3, 4, 5}}
	mylog := []LogEntry{{1, []byte("read")}, {2, []byte("cas")}}
	sm := StateMachine{myconfig: config, state: "FOLLOWER", currentTerm: 2, votedFor: 1, log: mylog, logCurrentIndex: 1, logCommitIndex: 1}

	// vote request from same term leader again with updated log. Should be granted
	action := sm.ProcessEvent(VoteRequestEvent{2, 1, 3, 2})
	...
	expectAction(t, action, []interface{}{Alarm{200}, Send{1, VoteResponseEvent{2, true}}}, "Error in TestVoteRequestFollower")

	// vote request from higher term leader with less updated log . should not be granted
	sm = StateMachine{myconfig: config, state: "FOLLOWER", currentTerm: 2, votedFor: 1, log: mylog, logCurrentIndex: 1, logCommitIndex: 1}
	action = sm.ProcessEvent(VoteRequestEvent{3, 1, 2, 1})
	expectStateMachine(t, sm, StateMachine{myconfig: config, state: "FOLLOWER", currentTerm: 3, votedFor: 0, log: mylog, logCurrentIndex: 1, logCommitIndex: 1}, "Error in TestVoteRequestFollower")
	expectAction(t, action, []interface{}{Send{1, VoteResponseEvent{3, false}}, StateStore{"FOLLOWER", 3, 0}}, "Error in TestVoteRequestFollower")

	//vote  request from higher term leader with more updated log . should be granted
	sm = StateMachine{myconfig: config, state: "FOLLOWER", currentTerm: 2, votedFor: 1, log: mylog, logCurrentIndex: 1, logCommitIndex: 1}
	action = sm.ProcessEvent(VoteRequestEvent{3, 1, 2, 3})
	expectStateMachine(t, sm, StateMachine{myconfig: config, state: "FOLLOWER", currentTerm: 3, votedFor: 1, log: mylog, logCurrentIndex: 1, logCommitIndex: 1}, "Error in TestVoteRequestFollower")
	expectAction(t, action, []interface{}{Alarm{200}, Send{1, VoteResponseEvent{3, true}}, StateStore{"FOLLOWER", 3, 1}}, "Error in TestVoteRequestFollower")

	//vote request from lower term leader, Should be rejected
	sm = StateMachine{myconfig: config, state: "FOLLOWER", currentTerm: 2, votedFor: 1, log: mylog, logCurrentIndex: 1, logCommitIndex: 1}
	action = sm.ProcessEvent(VoteRequestEvent{1, 1, 2, 3})
	expectStateMachine(t, sm, StateMachine{myconfig: config, state: "FOLLOWER", currentTerm: 2, votedFor: 1, log: mylog, logCurrentIndex: 1, logCommitIndex: 1}, "Error in TestVoteRequestFollower")
